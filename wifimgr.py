...
class WifiManager:
    ...
    def wants_accesspoint(cls) -> bool:
        ...
        #print("sta essid = ap essid ",cls.wlan().config("essid"))
        if cls._ap_start_policy in static_policies:
            return static_policies[cls._ap_start_policy]
        # By default, that leaves "Fallback"
#selfmade but error with accesspoint " cant set ap config"
        if cls.wlan().config("essid") != cls.ap_config["config"]["essid"]:
            return static_policies["always"]
